...
import { getCourses, getUserEnrollments, unenrollFromCourse, getCourse } from '../../core/api/courses'
...
function StudentDashboard() {
  ...
  const loadEnrolledCourses = async () => {
    ...
    try {
      setLoading(true)
      // DB에서 수강 목록 가져오기
      const enrollments = await getUserEnrollments(typeof user.id === 'number' ? user.id : Number(user.id))
      const enrolledCourses = await Promise.all(
        enrollments.map(enrollment => getCourse(enrollment.courseId))
      )
        ...
        <div className="space-y-4 md:space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[20%_1fr] gap-4 md:gap-6">
            ...
            <div className="flex flex-col gap-4">
              ...
              <div id="calendar-tooltip-area" className="card-panel p-4 flex flex-col items-center justify-center" style={{ height: '296px', minHeight: '296px' }}>
                {/* 기본 메시지는 calendar-tooltip-area가 비어있을 때만 표시됨 (StudentCalendar에서 createPortal로 내용을 렌더링하면 이 메시지는 숨겨짐) */}
                <div className="text-sm text-neutral-500 text-center calendar-default-message">
                  ...
                  >
                    ...
                    <div className="space-y-3">
                      ...
                      <div
                        ...
                        style={{ backgroundImage: `url('${normalizeThumbnailUrl(course.thumbnail, '/thumbnails/ccc.jpg')}')` }}
                      >
